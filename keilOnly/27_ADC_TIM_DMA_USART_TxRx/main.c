...
int main(void)
{
	...
	while(1)
	{
		...
		sprintf(volt_str,"%.2f",volt); // THIS TOOK 10uS
	
		userPrintf(volt_str);  // THIS TOOK 3MILLI SECONDS
		
		if(USART2->SR & USART_SR_RXNE)			// check tx speed 
		{
			USART2->SR &= ~USART_SR_RXNE;
			rec_data[x]=USART2->DR;
			if(++x>sizeof(rec_data)) x=0;
		}
	}
}
